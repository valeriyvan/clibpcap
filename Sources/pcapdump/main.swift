...
struct PcapDump: ParsableCommand {
    ...
    @Option(name: .customLong("srcip"), help: "Filter packets by source IP address (IPv4 or IPv6)")
    var srcIpFilter: String?

    @Option(name: .customLong("dstip"), help: "Filter packets by destination IP address (IPv4 or IPv6)")
    var dstIpFilter: String?
    ...
    func run() throws {
        ...
        while pcap_next_ex(handle, &headerPointer, &dataPointer) == 1 {
            ...
            if packetIndex < from - 1 { continue }

            if let to = to, packetIndex > to - 1 { break }
            ...
            if header.pointee.caplen >= 14 {
                ...
                if etherType == 0x0800, header.pointee.caplen >= 23 { // IPv4
                    ...
                    if let srcIpFilter, srcIpFilter != srcIP { continue }
                    if let dstIpFilter, dstIpFilter != dstIP { continue }
                    print("  Source: \(srcIP)")
                    ...
                    if let srcIpFilter {
                        if srcIpFilter != srcIP { continue }
                    }
                    let dstIP = compressIPv6(dstSegments)
                    if let dstIpFilter {
                        if dstIpFilter != dstIP { continue }
                    }
                    print("  Source: \(srcIP)")
                    print("  Destination: \(dstIP)")
                } else {
...
// Guaranteed to return lowercased
...
func isIPv6(_ ip: String) -> Bool {
    let segments = ip.split(separator: ":").map { String($0) }
    if segments.count > 8 { return false }
    for segment in segments {
        if segment.isEmpty { continue }
        if segment.count > 4 { return false }
        if !segment.allSatisfy({ $0.isHexDigit }) { return false }
    }
    return true
}

func uncompressIPv6(_ ip: String) -> [UInt16] {
    let segments = ip.split(separator: ":").map { String($0) }
    var result = [UInt16]()
    result.reserveCapacity(8)
    let zeroCount = 8 - segments.count
    for segment in segments {
        if segment.isEmpty {
            ...
            if let value = UInt16(segment, radix: 16) {
                result.append(value)
            }
        }
    }
    assert(result.count == 8, "Invalid IPv6 address")
    return result
}
